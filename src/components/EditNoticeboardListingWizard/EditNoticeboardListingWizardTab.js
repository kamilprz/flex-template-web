...
const redirectAfterDraftUpdate = (listingId, params, tab, marketplaceTabs, history) => {
  ...
  if (params.type === LISTING_PAGE_PARAM_TYPE_NEW) {
    const draftURI = createResourceLocatorString('EditNoticeboardListingPage', routes, currentPathParams, {});
    history.replace(draftURI);
  }

  // Redirect to next tab
  const nextPathParams = pathParamsToNextTab(currentPathParams, tab, marketplaceTabs);
  const to = createResourceLocatorString('EditNoticeboardListingPage', routes, nextPathParams, {});
  history.push(to);
};

const EditNoticeboardListingWizardTab = props => {
  const {
    tab,
    marketplaceTabs,
    params,
  ...
  switch (tab) {
    ...
    case FEATURES: {
      const submitButtonTranslationKey = isNewListingFlow
        ? 'Next:Location'
        : 'EditNoticeboardListingWizard.saveEditFeatures';
      return (
        <EditNoticeboardListingFeaturesPanel
          {...panelProps(FEATURES)}
          submitButtonText={intl.formatMessage({ id: submitButtonTranslationKey })}
    ...
    case POLICY: {
      const submitButtonTranslationKey = isNewListingFlow
        ? 'EditNoticeboardListingWizard.saveNewPolicies'
        : 'EditNoticeboardListingWizard.saveEditPolicies';
      return (
        <EditListingPoliciesPanel
          {...panelProps(POLICY)}
          submitButtonText={intl.formatMessage({ id: submitButtonTranslationKey })}
    ...
    case AVAILABILITY: {
      const submitButtonTranslationKey = isNewListingFlow
        ? 'EditNoticeboardListingWizard.saveNewAvailability'
        : 'EditNoticeboardListingWizard.saveEditAvailability';
      return (
        <EditListingAvailabilityPanel
...
EditNoticeboardListingWizardTab.defaultProps = {
  listing: null,
  updatedTab: null,
};
...
EditNoticeboardListingWizardTab.propTypes = {
  params: shape({
...
export default EditNoticeboardListingWizardTab;
